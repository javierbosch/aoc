...
for r in range(0,r_max):
    for c in range(0,c_max):
        if is_symbol(r,c):
            ...
            if lines[r][c] == '*' and len(unique_number_ranges) == 2:
                special += unique_number_ranges
                total += number_range_to_int(unique_number_ranges[0]) * number_range_to_int(unique_number_ranges[1])
            else:
                number_ranges += unique_number_ranges




unique_number_ranges = list(set(number_ranges) - set(special))
print(sum([number_range_to_int(x) for x in unique_number_ranges]) + total)
